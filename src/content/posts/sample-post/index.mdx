...
The `Compare` component puts a before-and-after pair side by side. The same image stands in for both here.

<Compare
  before={beforeAfter}
  beforeAlt="Two grey panels labelled Before and After, standing in for a before image"
  after={beforeAfter}
  afterAlt="Two grey panels labelled Before and After, standing in for an after image"
/>

## Link cards

A `LinkCard` for a GitHub repository, a pub.dev package, and a page that cannot be fetched (it falls back to a plain link):

<LinkCard url="https://github.com/koji-1009/shutter" />

<LinkCard url="https://pub.dev/packages/shutter" />

<LinkCard url="https://example.invalid/" />
